...
const DEFAULT_PROPCO_ASSUMPTIONS = {
  ...
  buildArea: 13000,
  buildCost: 11.5,
  includeMedEq: true,  capexMedEqQty: 1,
...
const runPropCoEngine = (assumptions, opCoModelData, config) => {
  ...
  const landCost =
    (assumptions.includeLand ?? true)
      ...
      : 0;
  const buildCost = (assumptions.buildArea * assumptions.buildCost) / 1000;  const infraCost =
  ...
  const ffeCost = assumptions.includeFFE
    ...
    : 0;
  const totalHardCosts = buildCost + infraCost + ffeCost + sharingDevCost;

  const coreCostForPct = buildCost + ffeCost + infraCost + sharingDevCost;
  ...
  const infraBasis = infraCost + sharingDevCost + (totalHardCosts > 0 ? (totalSoftCosts * (infraCost + sharingDevCost)) / totalHardCosts : 0);
  const ffeBasis =
    ffeCost +
  ...
  for (let i = 1; i <= projYears; i++) {
    ...
    const reserve = revenue * (assumptions.ffeReservePct / 100);    let deferredCapex = 0;

    const ebitda =
      revenue - maint - taxOp - overhead - reserve;
...
console.log('Dev CAR:', propCoData.totals.devCar);
